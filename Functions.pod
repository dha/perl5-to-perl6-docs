...
One general comment: Perl 6 takes its objects a lot more seriously
than Perl 5. In Perl 6, everything is an object, although the language
is flexible enough to not force you to work in an object oriented
manner if you so desire. What this does mean, however, is that a lot
of things that ar function calls of the form C<function(@args)> can
now method calls of the form C<@args.function>. This should be obvious
in the following text, but it probably behooves you to get into that
frame of mind now.
...
[NEEDS FURTHER RESEARCH] What C<caller> does in Perl 5 has been taken on
by C<callframe>. However, a simple call to C<callframe> will be of
little use, as it will return a CallFrame object, rather than any useful
...
way of getting the package name out of C<callframe> though.
...
Still seems to work as it does in Perl 5.
...
There is now a C<truncate> function (also usable as a method) that does
what Perl 5's C<int> does. You may want to use that as a direct
...
C<rand> by itself still works as it does in Perl 5, but you can no
longer give it an argument. You can, however, use it as a method on a
number to get that behavior. I. e. the Perl 5 C<rand(100)> is equivalent
to C<100.rand> in Perl 6. Additionally, you can get a random integer by
