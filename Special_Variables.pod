...
document is an attempt to guide you from the Special Variables in Perl 5 to
...
can also be achieved by using C<.say>.
...
which are more versatile. Placeholder variables are created with the C<^>
twigil (e. g. C<$^z>. They can be used in a bare block or in a subroutine
without an explicit parameter list. The arguments to the block are assigned to
the placeholder variables in their Unicode order. I. e. even if the variables
appear in the block in the order C<($^q, $^z, $^a)>, they will be I<assigned>
in the order C<($^a, $^q, $^z)>. Ergo:
